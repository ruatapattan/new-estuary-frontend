    ...
    <>
      {comment.map((commentItem) => (
        <Stack justifyContent='space-between' direction='row' spacing={1} mb='10px'>
          ...
          <Paper sx={{ padding: '10px', width: '100%' }} elevation={3}>
            {/*  */}
            <MainCommentContent content={commentItem.content} firstName={commentItem.firstName} />

            <Grid container spacing={5} mb='5px'>
              {/*  */}
              <MainCommentbottom commentItem={commentItem} postItem={postItem} />
            </Grid>
          </Paper>
        </Stack>
      ))}
