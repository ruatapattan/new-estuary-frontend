      ...
      axios
        ...
        .then(res2 => {
          if (user.id !== item.User.id) {
            alert('notify like');
            console.log(res2);
            sendNotification(user.id, user.username, item.User.id, 'liked', 'product', 'likeId', res2.data.likeId);
          }
        });
    } else {
      // alert("not first");
      ...
      >
        <Card sx={{ maxWidth: 345, height: '400px' }}>
          <CardActionArea onClick={() => history.push(`/product/${item.id}`)}>
            <CardMedia component="img" sx={{ width: 280, height: 258 }} image={item.coverPic} alt="green iguana" />
