...
function ProductDetail({
  ...
  const defaulfProfile =
    "https://res.cloudinary.com/dl7u9oybl/image/upload/v1635217850/img-placeholder_rutnat.jpg";
  ...
  followingLists.map((item) => {
    if (
      +item.followedId === +product.userId &&
    ...
    if (followingLists.length === 0) {
      ...
      followingLists.map((item) => {
        if (
          ...
          axios
            .put(`/following/${item.id}`, { isSubscribed: !isSubscribed })
            .then((res) => {
              setToggle((curr) => !curr);
            });
        } else {
          axios
            .post("/following", { followedId: product.userId })
            .then((res) => {
              setToggle((curr) => !curr);
            });
        }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Button
          variant="gradient"
          sx={{ borderRadius: "8px", p: "0px", mr: "10px", mb: "10px" }}
        >
